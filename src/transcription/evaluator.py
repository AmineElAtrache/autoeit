...
class TranscriptionEvaluator:
    ...
    def evaluate_csv(
        ...
        if hyp_col not in df.columns or ref_col not in df.columns:
            raise ValueError(
                f"CSV must contain '{hyp_col}' and '{ref_col}' columns"
            )
        
        samples = []
        for idx, row in df.iterrows():
            audio_file = row.get(audio_col, f"sample_{idx:03d}") if audio_col else f"sample_{idx:03d}"
    ...
    def _aggregate(self, samples: List[TranscriptionEvalResult]) -> TranscriptionEvaluation:
    ...
    def print_summary(self, evaluation: TranscriptionEvaluation) -> None:
        ...
        logger.info(f"Avg WER: {evaluation.avg_wer:.2%} (target: ≤{self.target_wer:.0%})")
...
def compare_transcriptions(
    ...
    comparison_results = []
    for idx, (b_row, t_row) in enumerate(zip(
        baseline_df.itertuples(),
        test_df.itertuples()
    )):
